...
public class Model {
    ...
    public static boolean attackSelected(int r, int c) {
        // : Make this actually work :D
        TerrainTile t = map.getTile(r, c);
        MapObject a = t.getOccupant();
        MilitaryUnit unit = (MilitaryUnit) selected.getOccupant();
        if (a != null) {
            if (a.getOwner() != playerCivilization && unit.getCanAttack()) {
                unit.attack(a);
                if (a.isDestroyed()) {
                    t.setOccupant(null);
                }
                //System.out.println("attack sucessful");
                return true;
            }
        }
        return false;
