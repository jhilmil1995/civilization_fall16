...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (state == GameState.ATTACKING) {
            boolean sucess = attack(last);
            if (!sucess) {
                Alert newAlert = new Alert(Alert.AlertType.INFORMATION);
                newAlert.setHeaderText("Could not attack!");
                newAlert.setTitle("Attacking enemy failed");
                newAlert.showAndWait();
            } else {
                String musicFile = ("src/main/java/controller/victory.wav");
                Media sound = new Media(new File(musicFile).toURI().toString());
                MediaPlayer mediaPlayer = new MediaPlayer(sound);
                mediaPlayer.play();
                //Alert newAlert = new Alert(Alert.AlertType.INFORMATION);
                //newAlert.setHeaderText("Attacking enemy sucessful");
                //newAlert.setTitle("Attacking enemy");
                //newAlert.showAndWait();
                //mediaPlayer.play();
            }
            ...
            if (move(last)) {
                lastClicked = last;
                String musicFile = ("src/main/java/controller/whoosh.wav");
                Media sound = new Media(new File(musicFile).toURI().toString());
                MediaPlayer mediaPlayer = new MediaPlayer(sound);
                mediaPlayer.play();
                /*Alert newAlert = new Alert(Alert.AlertType.INFORMATION);
                newAlert.setHeaderText("Moving unit sucessful");
                newAlert.setTitle("Moving unit");
                newAlert.showAndWait();*/
            } else {
                Alert newAlert = new Alert(Alert.AlertType.INFORMATION);
                newAlert.setHeaderText("Could not move unit!");
                newAlert.setTitle("Moving unit failed");
                newAlert.showAndWait();
            }
        }
        //update lastClicked
