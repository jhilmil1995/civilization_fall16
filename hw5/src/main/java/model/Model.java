...
public class Model {
    ...
    static class HappinessComparator implements Comparator<Civilization> {
        ...
        public int compare(Civilization a, Civilization b) {
            if (a.getHappiness() > b.getHappiness()) {
                return -1; // greater than
            } else if (a.getHappiness() < b.getHappiness()) {
                return 1; // less than
            } else {
                return 0; //a.getName().compareTo(b.getName());
            }
        }
    }

    static class TechPointComparator implements Comparator<Civilization> {

        public int compare(Civilization a, Civilization b) {
            if (a.getTechnology().getTechPoints()
                > b.getTechnology().getTechPoints()) {
                return -1; // greater than
            } else if (a.getTechnology().getTechPoints()
                < b.getTechnology().getTechPoints()) {
                return 1; // less than
            } else {
                return 0; //a.getName().compareTo(b.getName());
    ...
    static class TechMilitaryProwess implements Comparator<Civilization> {
        public int compare(Civilization a, Civilization b) {
            if (a.getStrategy().getStrategyLevel()
                > b.getStrategy().getStrategyLevel()) {
                return -1;
            } else if (a.getStrategy().getStrategyLevel()
                < b.getStrategy().getStrategyLevel()) {
                return 1;
            } else {
                return 0;
    ...
    public static void standings(int choice) {
        ...
        case 1:
            //Military Prowess
            //static <T exteds Comparable<? super T>> sort (List<T> list){}
            System.out.println();
            System.out.println("People with the Pointiest Sticks:");
            Collections.sort(civs, new TechMilitaryProwess());
            for (Civilization civ: civs) {
                System.out.println(i++ + ". " + civ.getName()
        ...
        case 3:
            ...
            Collections.sort(civs, new TechPointComparator());
            for (Civilization civ: civs) {
                System.out.println(i++ + ". " + civ.getName()
            ...
            Collections.sort(civs, new Comparator<Civilization>() {
                public int compare(Civilization a, Civilization b) {
                    if (a.getResources() > b.getResources()) {
                        return -1;
                    } else if (a.getResources() < b.getResources()) {
                        return 1;
                    } else {
                        return 0;
                    }
        ...
        case 5:
            ...
            for (Civilization civ: civs) {
                System.out.println(i++ + ". " + civ.getName() + "Settlements - "
                    + civ.getNumSettlements() + ": Military level - "
                    + civ.getStrategy().getStrategyLevel());
            }
            break;
        default:
